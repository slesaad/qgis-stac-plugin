...
from qgis_stac_plugin_x.conf import settings_manager
from qgis_stac_plugin_x.conf import ConnectionSettings
...
class SettingsManagerTest(unittest.TestCase):
    ...
    def test_read_write_settings(self):
        ...
        self.assertEqual(settings_manager.get_value("non_existing_key", "none"), "none")
        self.assertIsNone(settings_manager.get_value("non_existing_key"))
        settings_manager.set_value("name", "test")
        self.assertEqual(settings_manager.get_value("name"), "test")
        settings_manager.remove("name")
        self.assertIsNone(settings_manager.get_value("name"))

    def test_connection_settings(self):
        """Connection settings can be added, edited and removed"""

        self.assertEqual(settings_manager.get_value("connections", "none"), "none")
        ...
        self.assertIsNone(settings_manager.get_value(connection_name))

        settings_manager.save_connection_settings(connection)

        self.assertIsNotNone(settings_manager.get_value(connection_name))

        stored_connection = settings_manager.get_connection_settings(connection_id)
        self.assertEqual(connection.auth_config, stored_connection.auth_config)
        ...
        settings_manager.save_connection_settings(second_connection)
        second_stored_connection = settings_manager.get_connection_settings(
            second_connection_id
        )
        self.assertEqual(
            second_connection.auth_config, second_stored_connection.auth_config
        )
        ...
        settings_manager.set_current_connection(second_connection_id)
        current_connection = settings_manager.get_current_connection()

        self.assertTrue(settings_manager.is_current_connection(second_connection_id))

        self.assertEqual(second_connection.auth_config, current_connection.auth_config)
